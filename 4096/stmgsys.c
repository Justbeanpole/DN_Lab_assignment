	...
	while (s)
	{
		...
		if (strlen(input) == 1 && isdigit(input[0]))
		{
			...
			case 1: //입력
			{
				...
				{
					printf("학번: ");
					scanf_s("%[^\n]%*c", &a[i].num, sizeof(a[i].num)); //버퍼에 대기 중이던 엔터가 처리되서 그냥 넘어가버리는 현상 발생
...
void printst()
{
	while (1)
	{
		...
		else
		{
			for (int n = 0; n < i; n++)
			{
				//printf("학번: %s | 이름: %s | 국어점수: %d | 수학점수: %d | 영어점수: %d | 총합점수: %d | 평균점수: %d\n\n",
